	...
	<view class="Box" style="">
		...
		<view class="topBox anmt" :style="{color:fontColor,backgroundColor:menuBg,top:show?'0':'-150px'}">
			...
			<view style="height: 100upx;line-height: 100upx;text-overflow: ellipsis;" class="tMain">
				<image src="../../static/back.svg" class="back" @click="back()"></image>
		...
		getText(param){
			...
			this.getHtml({
				...
				success: function(res){
					...
					this_.section_title =  $('#mains').find('h1').text();
					var text = this_.section_title + `<br><br>`;
					$('#book_text').find('p').each(function(e,w){
						text += `&nbsp;&nbsp;&nbsp;&nbsp;` + $(this).text() + `<br><br>`
					})
					this_.content_text = text
					this_.shuming = $('#mains').find('span').eq(1).text()
					this_.nextUrl = this_.$bookUrl + $(".book_content_text_next").find('a').eq(2).attr('href')
